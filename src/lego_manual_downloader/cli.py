...
def process_owned_sets(sets: list[LegoSet], download_path: Path, db: ManualDb, providers: ProviderFactory):
    """Download a manual for each owned set that is not already in the database."""
    if not sets:
        print("No owned sets found.")
        return
    for lego_set in sets:
        print(f"Processing {lego_set.number} - {lego_set.name} ({lego_set.year})")
        ...
        if providers.download_manual(lego_set, download_path / lego_set.file_name):
            ...
            print(f"Unable to download manual for {lego_set.number} - {lego_set.name} ({lego_set.year})")
    db.write_db()

def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser()
    parser.add_argument("download_dir", help="Path to directory to write manuals to.", type=Path)
    parser.add_argument("--config", help="Path to config file.", default=None, required=False, type=Path)
    return parser
